...
from utils.helper import mode_from_str
...
class ScorePost(commands.Cog):
    ...
    async def recent_score(
        ...
        player_id = Database.execute_query(
            'SELECT `id` '
            'FROM `users` '
            f'WHERE `discord_id` = {interaction.user.id}',
            res=True
        )
        if not player_id:
            return await interaction.response.send_message('Couldn\'t find your Discord ID, please contact our developer.')
        ...
        embed = nextcord.Embed(
            ...
            description=
            f"""
            • **{GRADE_EMOJI[score['grade']]}** • **{round(score['acc'], 2)}pp**
            • {round(score['acc'], 2)}%
