...
class ScorePost(commands.Cog):
    ...
    async def recent_score(
        ...
        if converted_mode is None or converted_mode not in range(0, 12):
            return await interaction.response.send_message(embed=nextcord.Embed(description='Selected wrong modes, available modes - `vn!std`, `vn!taiko`, `vn!catch`, `vn!mania`, `rx!std`, `rx!taiko`, `rx!catch`, `rx!mania`, `ap!std`, `ap!taiko`, `ap!catch`, `ap!mania`.'))
        ...
        if not player_id:
            return await interaction.response.send_message(embed=nextcord.Embed(
                description='Couldn\'t find player, if you sure that a mistake, please contact our developer.'
            ))
        ...
        if data.status_code != 200:
            if get(f'https://api.{OSU_DOMAIN}/v1/get_player_count', verify=False).status_code != 200:
                return await interaction.response.send_message(embed=nextcord.Embed(
                    description='Get critical error, seems like osu! server is down.'
                ))
            return await interaction.response.send_message(embed=nextcord.Embed(
                description='Couldn\'t find any score.'
            ))
        ...
        if score_data['scores'] == []:
            return await interaction.response.send_message(
                embed=nextcord.Embed(
                    description='Couldn\'t find any score for selected mods.'
                ))
